...
def json_to_Dataset_ensemble(filepath: str) -> Dataset:
    ...
    for i in data:
        ...
        albert_attention_masks.append(
            [1 for i in range(len(i["albert_inputids"]))]
        )
        distilbert_attention_masks.append(
            [1 for i in range(len(i["distilbert_inputids"]))]
        )
    ...
    dataset = Dataset.from_dict(
        {
            ...
            "albert_inputids": albert_inputids,
            "distilbert_inputids": distilbert_inputids,
            "albert_wordids": albert_wordids,
            "distilbert_wordids": distilbert_wordids,
            "albert_attention_masks": albert_attention_masks,
            "distilbert_attention_masks": distilbert_attention_masks,
            "distilbert_token_labels": distilbert_token_labels,
            "albert_token_labels": albert_token_labels,
        }
    )
...
def split_list_by_none(input_list, outputs):
    ...
    for ind, item in enumerate(input_list):
        ...
        if item is None and ind != 0 and input_list[ind - 1] != None:
            ...
            outputsnew.append(
                outputs[ind - len(current_sublist) + 1 : ind + 1]
            )
            current_sublist = []
...
def ensembler_batch(output1, output2, word_ids1, word_ids2):
    ...
    for i in range(len(word_ids1)):
        new_output1, new_output2 = ensembler(
            output1[i], output2[i], word_ids1[i], word_ids2[i]
        )
