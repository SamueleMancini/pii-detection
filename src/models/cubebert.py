...
from transformers import DistilBertForTokenClassification, AlbertForTokenClassification
from src.utils.labels import label2id, id2label, n_labels
...
class CubeBert(nn.Module):
    ...
    def forward(self,
        ...
        alpha = alpha.view(1, 1, -1)  # broadcast to [batch_size, seq_len, n_labels]
        ...
        combined_logits = torch.zeros((batch_size, max_words, self.num_labels), device=device)
        ...
        if labels is not None:
            ...
            loss = loss_fn(combined_logits.view(-1, self.num_labels), labels.view(-1))
