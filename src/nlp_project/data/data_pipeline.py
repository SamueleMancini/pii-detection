...
from nlp_project.utils import label2id
...
def tokenise_and_align(doc, bio, tokenizer, label2id):
    enc = tokenizer(
        [t.text for t in doc], is_split_into_words=True, truncation=True
    )
...
def build_records(raw):
    ...
    d_ids, d_wids, d_lbls, word_labels = tokenise_and_align(
        doc, bio, tok_distil, label2id
    )
    a_ids, a_wids, a_lbls, _ = tokenise_and_align(
        doc, bio, tok_albert, label2id
    )
