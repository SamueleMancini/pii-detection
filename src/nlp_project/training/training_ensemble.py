...
from nlp_project.data import build_collate_fn, json_to_Dataset_ensemble
from nlp_project.models import CubeBert
from nlp_project.utils import compute_metrics
...
train_ds = json_to_Dataset_ensemble(
    os.path.join(ENSEMBLE_DATA, "ensemble_train.json")
)
val_ds = json_to_Dataset_ensemble(
    os.path.join(ENSEMBLE_DATA, "ensemble_val.json")
)
...
val_loader = DataLoader(
    val_ds, batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_fn
)
...
def train_ensemble(
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            f"Val F1: {metrics['f1']:.4f} | Precision: {metrics['precision']:.4f} | Recall: {metrics['recall']:.4f}"
        )

        # ---- Alpha inspection
        if log_alphas:
            alpha_values = (
                torch.sigmoid(model.alpha_logits).detach().cpu().numpy()
            )
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(
                model.state_dict(), os.path.join(output_dir, "CubeBert.pt")
            )
